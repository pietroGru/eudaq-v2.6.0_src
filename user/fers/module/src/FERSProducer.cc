...
namespace {
  auto dummy0 = eudaq::Factory<eudaq::Producer>::Register<
      FERSProducer, const std::string &, const std::string &>(
      FERSProducer::m_id_factory);
}
//----------DOC-MARK-----END*REG-----DOC-MARK----------

FERSProducer::FERSProducer(const std::string &name,
...
void FERSProducer::DoInitialise() {
  ...
  if (ret == 0) {
    ...
    EUDAQ_THROW("unable to connect to fers with ip address: " +
                fers_ip_address);
  ...
  // << " "<<fers_id<<std::endl;
  EUDAQ_INFO("Connected to handle " + std::to_string(handle) + " ip " +
             fers_ip_address + " " + fers_id + " connectedboards " +
             std::to_string(shmp->connectedboards));
  EUDAQ_WARN("check shared on board " + std::to_string(brd) + ": " +
             std::string(shmp->IP[brd]) + "*" + std::string(shmp->desc[brd]) +
...
void FERSProducer::DoConfigure() {
  ...
  fers_final_filename = conf->Get("FERS_MODIFIED_FOLDER", "") + "FERS_" +
                        std::to_string(m_plane_id) + "_run_" +
                        std::to_string(GetRunNumber()) + "_" +
                        std::to_string(current_time) + ".txt";
  fers_acq_mode = conf->Get("FERS_ACQ_MODE", 0);
  // std::cout<<"in FERSProducer::DoConfigure, handle = "<< handle<< " file
  // "<<fers_final_filename<< " m_LG_Gain "<<m_LG_Gain<< std::endl;
  ...
  if (input_file.good() && new_file.good()) {
    ...
    for (std::string input_line; std::getline(input_file, input_line);) {
      if ((input_line.find("LG_Gain") != std::string::npos) && (m_LG_Gain > 0))
        input_line =
            "LG_Gain                            " + std::to_string(m_LG_Gain);
      if ((input_line.find("HG_Gain") != std::string::npos) && (m_HG_Gain > 0))
        input_line =
            "HG_Gain                            " + std::to_string(m_HG_Gain);
      new_file << input_line << "\n";
  ...
  int ret_dummy = 0;
  // std::cout << "\n**** FERS_HV_Imax from config: "<< fers_hv_imax <<
  // std::endl;
  ...
  // std::cout << "\n**** FERS_HV_Vbias from config: "<< fers_hv_vbias <<
  // std::endl;
...
void FERSProducer::DoStartRun() {
  ...
  EUDAQ_INFO("FERS_ReadoutStatus (0=idle, 1=running) = " +
             std::to_string(FERS_ReadoutStatus));
}
...
void FERSProducer::RunLoop() {
  ...
  while (!m_exit_of_run) {
    ...
    // Make unique create a RawEvent type object and assign the 'fers' argument
    // to the extend word and description
    ...
    // real data
    //
    status = FERS_GetEvent(vhandle, &bindex, &DataQualifier, &tstamp_us, &Event,
                           &nb);
    ...
    if (DataQualifier > 0) {
      ...
      // std::cout<<"--FERS_ReadoutStatus (0=idle, 1=running) = " <<
      // FERS_ReadoutStatus <<std::endl; std::cout<<"--status of FERS_GetEvent
      // (0=No Data, 1=Good Data 2=Not Running, <0 = error) = "<<
      // std::to_string(status)<<std::endl; std::cout<<"  --bindex = "<<
      // std::to_string(bindex) <<" tstamp_us = "<< std::to_string(tstamp_us)
      // <<std::endl; std::cout<<"  --DataQualifier = "<<
      // std::to_string(DataQualifier) +" nb = "<< std::to_string(nb)
      // <<std::endl;
      ...
      //  std::cout<<"producer > " << "x_pixel: " <<	x_pixel << " y_pixel: "
      //  << y_pixel << " DataQualifier : " <<	DataQualifier << std::endl;
      double run_time = ev->GetTimestampEnd() - ev->GetTimestampBegin();
      FERSpack_CLEAR_event(Event, m_plane_id, GetRunNumber(), trigger_n,
                           (double)ev->GetTimestampEnd(),
                           (double)ev->GetTimestampBegin(), data);
