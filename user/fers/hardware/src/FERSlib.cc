...
static int FUP_WriteFPGAFirmwareOnFlash(int handle, char* pageText, int textLen, void(*ptr)(char* msg, int progress))
{
	...
	for (int cnk = 0; cnk < (int)n_chunks; cnk++) {
		...
		} while ((crc_R != crc_T) && (cnt < 20)); // ripeto la lettura del CRC (10 volte) finch� non � uguale a quello calcolato per essere sicura che il pacchetto � tutto nella memoria del uBlaze
		if (cnt == 20) {
