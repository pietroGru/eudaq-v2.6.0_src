...
private:
  uint32_t m_noprint, m_nproducers, m_received_number;
  std::mutex m_mtx_map;
  std::map<std::string, int> received_types;
...
void FERSDataCollector::DoConfigure(){
  ...
  if(conf){
	...
	m_nproducers = conf->Get("COLLECTOR_NPRODUCERS", 1);
  }
}
...
void FERSDataCollector::DoReceive(eudaq::ConnectionSPC idx, eudaq::EventSP evsp){
   uint32_t id = eudaq::str2hash(idx->GetName()); 
   std::unique_lock<std::mutex> lk(m_mtx_map);
   std::map<std::string, std::string> event_tags =  evsp->GetTags();
   for (std::map<std::string, std::string>::iterator tag_iter = event_tags.begin(); tag_iter!=event_tags.end(); tag_iter++)
   {
	   if (tag_iter->first != "Detector_ID") continue;
	   std::string map_tag=tag_iter->first+tag_iter->second;
	   
	   if (received_types[map_tag] == 1) return;
	   else
	   { 
		   received_types[map_tag] = 1;
		   m_received_number++;
	   }
   }
  ...
  BuildEvent_Final();
   ...
   if(m_received_number==m_nproducers){
	   received_types.clear();
	   m_received_number=0;
		//call here some kind of trigger command   
   }
}
